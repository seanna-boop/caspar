...
tb_chem <- lapply(tb_chem, mutate, datetime = as.POSIXct(datetime,
                                                         format="%m/%d/%Y %H:%M", 
                                                         tz="Etc/GMT+7"))
#DOES THIS NEED TO B HERE bc i think datetime is already in posixct format -4.18.20????
...
tb_merge <- lapply(as.list(subws_names), function(subws_name) {
  ...
  if(length(idx_flo[[1]]) & length(idx_chem[[1]])) {
    ...
    res1 <- flo %>%
      ...
      summarise(firstq = Q[1],
                lastq = Q[length(Q)],
                volume_cf = sum(Q * as.numeric(dlt, 'secs')),
                rain_mm = sum(rnf_mm),
                mean_Q_cfs = mean(Q),
                sd_Q_cfs = sd(Q),
                min_Q_cfs = min(Q),
...
# (concentration, uS/cm, pH, NTU) and creates (ec_ravg:doc_ravg)
    ...
    out <- left_join(res2, res1, by = "date_group")
...
lapply(names(tb_merge), function(subws_name) {
  ...
  if(length(tb)) {
    ...
    write_csv(x = tb, path = file.path(dir_out, "flux_files" , paste0(subws_name, "_flux_1-1.csv")))
  }
})
