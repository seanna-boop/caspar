...
tb_merge <- lapply(as.list(subws_names), function(subws_name) {
  ...
  #   View(flo) #datetimes, DATE, and TIME all match here........
  # }})

    # create attribute with sub-watershed in flow table
    flo$subws <- subws_name
    View(flo) # has subws column now
    # }})
  ...
  # }})
    ...
    # round time
    chem$time_round <- round_date(chem$datetime, unit="10 mins")
    print(class(chem$time_round))
    # chem$time_round <- mutate(chem$time_round = as.POSIXct(chem$time_round,
    #                       format="%m/%d/%Y %H:%M", 
    #                       tz="Etc/GMT+7"))
    # class(chem$time_round)
    # View(chem)
    # }})
    
    # find subwatershed code in chem file, also check if it is within flow range
    chem_subws_idx <- which(grepl(chem$SUBWS, pattern=subws_name, ignore.case = TRUE) & chem$round_date %in% flo$datetime)
    ...
    res1 <- flo %>%
      ...
      mutate(volume_cf = sum(Q * as.numeric(dlt, 'secs')),
                rain_mm = sum(rnf_mm),
                mean_Q_cfs = mean(Q),
                sd_Q_cfs = sd(Q),
                min_Q_cfs = min(Q),
...
# (concentration, uS/cm, pH, NTU) and creates (ec_ravg:doc_ravg)
    ...
    out <- left_join(res2, res1, by = c("time_round" = "datetime"))
...
lapply(names(tb_merge), function(subws_name) {
  ...
  if(length(tb)) {
    ...
    write_csv(x = tb, path = file.path(dir_out, "flux_files" , paste0(subws_name, "_flux_k72.csv")))
  }
})
