...
# (concentration, uS/cm, pH, NTU) and creates (ec_ravg:doc_ravg)
    ...
    out <- left_join(res2, res1, by = c("date_group" = "date_group"))
...
lapply(names(tb_merge), function(subws_name) {
  ...
  if(length(tb)) {
    ...
    write_csv(x = tb, path = file.path(dir_out, "flux_files" , paste0(subws_name, "_flux_k7_orig_summ_mirror_join.csv")))
  }
})
