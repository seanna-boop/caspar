...
tb_rain <- lapply(tb_rain, mutate,  datetime = as.POSIXct(datetime, 
                                                          # note lowercase y in date format <<...no
                                                          format="%m/%d/%Y %H:%M", 
                                                          tz="Etc/GMT+7"))
...
tb_merge <- lapply(as.list(subws_names), function(subws_name) {
  ...
  if(length(idx_flo[[1]]) & length(idx_chem[[1]])) {
    ...
    #rename datetime to datetime_flo
    flo <- mutate(datetime_flo = datetime)
...
lapply(names(tb_merge), function(subws_name) {
  ...
  if(length(tb)) {
    ...
    write_csv(x = tb, path = file.path(dir_out, "flux_files" , paste0(subws_name, "_flux2.csv")))
  }
})
