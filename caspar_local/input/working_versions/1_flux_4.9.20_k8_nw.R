...
tb_flo <- lapply(tb_flo, mutate, 
                  datetime = as.POSIXct(paste0(DATE, " ", TIME),
                                        ...
                                        tz="Etc/GMT+7"))
View(tb_flo) #list
View(tb_flo[[1]]) # DATE, TIME, and datetime columns are consistent here....
...
tb_merge <- lapply(as.list(subws_names), function(subws_name) {
  ...
  #   print(class(chem$time_round)) # POSIXct
  # }})
...
lapply(names(tb_merge), function(subws_name) {
  ...
  if(length(tb)) {
    ...
    write_csv(x = tb, path = file.path(dir_out, "flux_files" , paste0(subws_name, "_flux_k8.csv")))
  }
})
